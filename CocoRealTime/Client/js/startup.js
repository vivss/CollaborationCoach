...
function init() 
{
	...
	signaling_socket.on('session_key', function(data)
	{
		...
		console.log("recieved the session key from the server: " + session_key);
		
		//*******************************************************************************
		/* THIS COULD BE NECESSARY, DEPENDS ON YOUR ANALYSIS!
		   We used this to setup a session_key/user_name relationship used in our
		   analysis scripts, so depends on your analysis needs.
	...
	// POSSIBLY NECESSARY, DEPENDS ON YOUR ANALYSIS, we had to delegate one client to run the 
	// analysis startup.
